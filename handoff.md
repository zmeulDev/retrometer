...
### Sistem de design (temă centralizată, paletă „Rally")

Refactor strict-UI (21 iun. 2026): culorile, tipografia și `InputDecoration`-ul hardcodate inline în cele ~2900 de linii de UI au fost centralizate într-un singur fișier — `lib/theme/retrometer_theme.dart`. Ecranele referă `RetrometerColors.*` / `RetrometerTextStyles.*` (namespaces statice) în loc de literalii `Colors.black`/`greenAccent`/`white70`…; `InputDecoration`-ul duplicat de ~11× a fost mutat în `inputDecorationTheme`; component themes (AppBar, FilledButton, FAB, BottomSheet, Dialog, Divider, Switch, IconButton, ProgressIndicator) unifică aspectul pe iOS + Android. Font family lăsat la platform-default (SF pe iOS, Roboto pe Android) — fără `google_fonts`, fără assets de font. Nicio logică/provideri/modele/math/chei prefs atinse; `flutter analyze` 0 issues, `flutter test` 31/31.

⚠️ **Deviație de la planul UI original** (`~/.claude/plans/jazzy-wobbling-moonbeam.md`): planul aprobase o paletă *aproape-neagră derivată din culorile existente*, cu `primaryAccent = #69F0AE greenAccent` „păstrat ca accent de brand". Implementarea a adoptat în schimb o paletă **„Rally"** (blue-gray desaturat, calmer decât neon greenAccent), decizie confirmată ulterior ca direcție de design dorită:

- `background #2D2E36`, `surface #363740`, `surfaceElevated #42434D`, `surfaceHeader #1F2A26` (teal-tinted), `sheet #3A3B44` — scara de suprafețe desaturată („Rally #33333D", întunecată pentru condus noaptea).
- `primary #41DFB4` (teal Rally, **nu** greenAccent), `secondary #FFEE58` (galben, pentru starea Δ întârziere), `danger #FF5252` (roșu Rally).
- Benzi Δ: `onTimeBg #0E4D38` + `onTimeFg #41DFB4` (teal, retonat din `#1B5E20` ca să se alinieze cu primary); `advanceBg #B71C1C` + `advanceFg #FF5252`; `delayBg #F57F17` + `delayFg #FFEE58` (avans/întârziere păstrate — funcționează, sunt identity cu vechile).
- Text pe dark: scara white-opacity păstrată (`textPrimary` white → `textFaint` white24, `hint` white30); linii `divider` white12, `dividerStrong` white24, `fieldBorder` white38.
- Butoane acțiune: `startFill #41DFB4` (teal), `stopFill #FF5252`, `resetFill #42434D` cu text `onActionFill` negru.
...
- `lib/main.dart` — `ProviderScope`, `MaterialApp` dark cu `theme: retrometerTheme()`. **Orientare liberă**: `setPreferredOrientations(DeviceOrientation.values)` — aplicabilă în portrait sau landscape, pe telefon sau tabletă (înainte forța landscape).
- `lib/theme/retrometer_theme.dart` — sistem centralizat de design: clasa `RetrometerColors` (paleta Rally, secțiunea de mai sus), `RetrometerTextStyles` (stiluri numite cu `FontFeature.tabularFigures()` pe numericile mari — Δ, distanță, elapsed), `retrometerTheme()` (`ThemeData` dark + component themes), `pillDecoration(color)` pentru badge-uri status, extensie `BuildContext` (`theme`/`textTheme`/`colorScheme`). Toate ecranele citesc culorile/tipografia de aici.
